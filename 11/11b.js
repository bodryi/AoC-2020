...
const step = {
  ...
  upright: (row, column) => [row - 1, column + 1],
  right: (row, column) => [row, column + 1],
  downright: (row, column) => [row + 1, column + 1],
  down: (row, column) => [row + 1, column],
  downleft: (row, column) => [row + 1, column - 1],
  left: (row, column) => [row, column - 1],
  upleft: (row, column) => [row - 1, column - 1],
};

const getOccupiedFirstSeatsCount = (row, column, layoutState) => {
  let occupiedCount = 0;

  Object.keys(step).forEach((key) => {
    let [i, j] = step[key](row, column);
    while (true) {
      const seatState = layoutState[i] && layoutState[i][j];

      if (seatState) {
        if (seatState === OCCUPIED_SEAT) {
          occupiedCount++;
          break;
        } else if (seatState === EMPTY_SEAT) {
          break;
        }
      } else {
        break;
      ...
      const coordinates = step[key](i, j);
      i = coordinates[0];
      j = coordinates[1];
    }
  });
...
const getEmptyLayout = (rows, columns) =>
  new Array(rows).fill(null).map(() => new Array(columns));

const copyLayout = (layout) => JSON.parse(JSON.stringify(layout));

const areLayoutsEqual = (l1, l2) => JSON.stringify(l1) === JSON.stringify(l2);

const iteration = (layoutState) => {
  const newState = getEmptyLayout(layoutState.length, layoutState[0].length);
  for (let row = 0; row < layoutState.length; row++) {
    for (let column = 0; column < layoutState[row].length; column++) {
      const seatState = layoutState[row][column];
      const occupiedCount = getOccupiedFirstSeatsCount(
        row,
        column,
        layoutState,
      );

      if (seatState === EMPTY_SEAT && !occupiedCount) {
        newState[row][column] = OCCUPIED_SEAT;
      } else if (seatState === OCCUPIED_SEAT && occupiedCount >= MIN_OCCUPIED) {
        newState[row][column] = EMPTY_SEAT;
      } else {
        newState[row][column] = layoutState[row][column];
      }
    }
  }
...
let prevState = copyLayout(layout);
let nextState = null;
let counter = 0;
while (!areLayoutsEqual(prevState, nextState)) {
  if (nextState) {
    prevState = copyLayout(nextState);
  }
  nextState = iteration(prevState);
  counter++;
}

let result = nextState.reduce(
  (acc, row) =>
    acc +
    row.reduce((cAcc, seat) => (seat === OCCUPIED_SEAT ? cAcc + 1 : cAcc), 0),
  0,
);
