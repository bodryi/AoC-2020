...
const parsedInput = input.split('\n').map((row) =>
  row.split(':').reduce((acc, item, index) => {
    ...
    if (index) {
      ...
      acc.letter = rule[1];
      const [min, max] = rule[0].split('-').map((n) => +n);
      acc.min = min;
      acc.max = max;
    }
    return acc;
...
const testPassword = (passwordRecord) => {
  const count = passwordRecord.password.split('').reduce((acc, symbol) => {
    if (symbol === passwordRecord.letter) {
      acc++;
    }
    return acc;
  }, 0);

  return count <= passwordRecord.max && count >= passwordRecord.min;
};
