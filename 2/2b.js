...
const input = `2-6 c: fcpwjqhcgtffzlbj
6-9 x: xxxtwlxxx
5-6 w: wwwwlwwwh
...
const parsedInput = input.split('\n').map((row) =>
  row.split(':').reduce((acc, item, index) => {
    item = item.trim();
    if (index) {
      acc.password = item;
    } else {
      ...
      acc.letter = rule[1];
      const [i1, i2] = rule[0].split('-').map((n) => +n);
      acc.index1 = i1 - 1;
      acc.index2 = i2 - 1;
    }
    return acc;
...
const testPassword = (passwordRecord) => {
  const passwordArray = passwordRecord.password.split('');

  return (
    (passwordArray[passwordRecord.index1] === passwordRecord.letter &&
      passwordArray[passwordRecord.index2] !== passwordRecord.letter) ||
    (passwordArray[passwordRecord.index1] !== passwordRecord.letter &&
      passwordArray[passwordRecord.index2] === passwordRecord.letter)
  );
};

const result = parsedInput.reduce((acc, password) => {
