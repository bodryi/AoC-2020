...
const containsColor = (rule, color) => {
  if (rule.children.find((r) => r.includes(color))) {
    return true;
  }
  ...
  return rule.children
    .map((c) => rules.find((r) => c.includes(r.parent)))
    .find((r) => containsColor(r, color));
};

const result = rules.filter((r) => !!containsColor(r, SHINY_GOLD)).length;
