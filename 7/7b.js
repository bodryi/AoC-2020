...
const input = `light salmon bags contain 5 dotted olive bags, 4 wavy lavender bags.
...
const rules = input.split('\n').map((rule) => {
  const splitRule = rule.split('bags contain').map((p) => p.trim());
  return {
    parent: splitRule[0],
    children: splitRule[1].split(',').map((p) => p.trim()),
  };
});

const shinyGoldRule = rules.find((r) => r.parent.includes(SHINY_GOLD));

const getBagNesting = (rule) => {
  if (rule.children.length === 1 && rule.children[0].includes(EMPTY_BAG)) {
    return 0;
  }

  return rule.children
    .map((c) => ({
      rule: rules.find((r) => c.includes(r.parent)),
      count: +c.split(' ')[0],
    }))
    .reduce((acc, r) => {
      return acc + r.count + r.count * getBagNesting(r.rule);
    }, 0);
};

console.log(getBagNesting(shinyGoldRule));
